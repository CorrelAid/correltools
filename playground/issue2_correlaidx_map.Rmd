...
cax_links <- 
  map_chr(
    c("berlin/", "bremen/", "cologne/", "dortmund/", "goettingen/", "hamburg/", "karlsruhe/", "konstanz/", "leipzig/", 
      "mannheim/", "munich/", "netherlands/", "paris/", "rhein-main/", "ruhrgebiet/", "stuttgart/", "switzerland/"),
    ~paste0("https://correlaid.org/correlaid-x/", .x)
    ) %>% 
  tibble(
    link = ., 
    chapter = c("Berlin", "Bremen", "Cologne/Bonn", "Dortmund", NA, "Hamburg", "Karlsruhe", "Konstanz", 
                "Leipzig", "Mannheim", "Munich", "Nederland", "Paris", "Rhein-Main", NA, "Stuttgart", "Switzerland")
    )
...
cax_map <- 
  ...
  addLegend(
    data = cities,
    "bottomright", pal = pal_cities, values = ~factor(year_founded),
    title = "Year founded",
    opacity = 1
  ) %>% 
